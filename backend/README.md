...
图纸编辑接口：`GET /api/drawings/{file_name}` 返回 `planned_paths` 和米制 `segments`，供 App 图纸编辑器
加载已有图元；`POST /api/drawings/save` 接收 `segments`。编辑已有图纸时同时传入 `overwrite: true` 和
`file_name`，后端会沿用原图纸基名创建新版本并返回新的 `file_name`，原版本保留不覆盖。
