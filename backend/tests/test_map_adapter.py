...
class PlannerCadPathTest(unittest.TestCase):
    def test_planner_uses_relative_json_file_name(self) -> None:
        path = planner_cad_path("circle_0500.json")

        self.assertEqual(path, "circle_0500.json")

    def test_planner_rejects_absolute_or_nested_file_path(self) -> None:
        with self.assertRaises(ValueError):
            planner_cad_path("/tmp/xline-cad/circle_0500.json")
        with self.assertRaises(ValueError):
            planner_cad_path("nested/circle_0500.json")
