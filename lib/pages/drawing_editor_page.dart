...
class _DrawingEditorPageState extends State<_DrawingEditorPage> {
  ...
  Offset selectionGestureCenter = Offset.zero;
  ...
  bool templateLibraryExpanded = true;
  bool _hasUnsavedChanges = false;

  bool get _editingExistingDrawing => editingFileName != null;

  String get _editableBaseName {
    final fileName = editingFileName!;
  ...
  void initState() {
    super.initState();
    editingFileName = widget.initialFileName;
    if (_editingExistingDrawing) {
      unawaited(_loadExistingDrawing());
    }
  }
  ...
  void _fitDrawingToCanvas() {
    ...
    final points = [
      for (final path in paths) ...path.points,
    ];
    ...
    final availableWidth = math.max(canvasSize.width - 72, 80);
    final availableHeight = math.max(canvasSize.height - 72, 80);
    final fittedScale = math.min(
      availableWidth / width,
      availableHeight / height,
    ).clamp(20.0, 96.0);
    setState(() {
      pixelsPerMeter = fittedScale;
      viewController.value = Matrix4.identity();
      selectedIndex = null;
  ...
  void _gestureStart(ScaleStartDetails details) {
    ...
    final hit = _nearestPath(_toWorld(details.localFocalPoint));
    final pinchOnSelection =
        details.pointerCount > 1 &&
        tool == _DrawingTool.select &&
        selected != null &&
        hit != null &&
        _selectionContains(hit);
    transformingSelection = pinchOnSelection;
    transformingView = details.pointerCount > 1 && !pinchOnSelection;
    ...
    if (transformingSelection) {
      ...
      selectionGroupStartPoints = {
        for (final index in _selectionIndices())
          index: List<Offset>.from(paths[index].points),
      };
      final points = selectionGroupStartPoints!.values.expand((value) => value);
      final minX = points.map((point) => point.dx).reduce(math.min);
      final maxX = points.map((point) => point.dx).reduce(math.max);
      final minY = points.map((point) => point.dy).reduce(math.min);
      final maxY = points.map((point) => point.dy).reduce(math.max);
      selectionGestureCenter = Offset((minX + maxX) / 2, (minY + maxY) / 2);
      setState(() => status = '双指捏合可直接缩放选中图形');
      return;
    }
    if (transformingView) {
      draft = null;
  ...
  void _gestureUpdate(ScaleUpdateDetails details) {
    if (transformingSelection) {
      ...
      final factor = details.scale.clamp(0.05, 20.0).toDouble();
      for (final entry in selectionGroupStartPoints!.entries) {
        paths[entry.key].points = entry.value
            .map(
              (point) =>
                  selectionGestureCenter +
                  (point - selectionGestureCenter) * factor,
            )
            .toList();
      }
      setState(
        () => status = '正在按 ${(factor * 100).toStringAsFixed(0)}% 缩放选中图形',
      );
      return;
    }
  ...
  void _start(DragStartDetails details) {
    ...
    if (tool == _DrawingTool.select) {
      dragStart = rawWorld;
      final hit = _nearestPath(rawWorld);
      final keepWholeSelection =
          wholeShapeSelected &&
          hit != null &&
          selectedIndex != null &&
          _selectionContains(hit);
      wholeShapeSelected = keepWholeSelection;
      selectedIndex = hit;
      selectionStartPoints = selectedIndex == null
          ? null
          : List<Offset>.from(paths[selectedIndex!].points);
      selectionGroupStartPoints = selectedIndex == null
          ? null
          : {
              for (final index in _selectionIndices())
                index: List<Offset>.from(paths[index].points),
  ...
  void _end(DragEndDetails details) {
    if (draft != null && draft!.points.length > 1) {
      ...
      if (length > 0.02) {
        _recordChange();
        paths.addAll(_groupLoadedSegments([draft!.points]));
      }
    }
    draft = null;
    selectionDragRecorded = false;
  ...
  bool _selectionContains(int index) {
    if (selectedIndex == null) return false;
    if (!wholeShapeSelected) return index == selectedIndex;
    final groupId = paths[selectedIndex!].groupId;
    return groupId == null
        ? index == selectedIndex
        : paths[index].groupId == groupId;
  }

  List<int> _selectionIndices() {
    final selected = selectedIndex;
    if (selected == null) return const [];
    if (!wholeShapeSelected) return [selected];
    final groupId = paths[selected].groupId;
    if (groupId == null) return [selected];
    return [
      for (var index = 0; index < paths.length; index++)
        if (paths[index].groupId == groupId) index,
    ];
  }

  void _selectWholeShape() {
    final position = doubleTapPosition;
    if (position == null || tool != _DrawingTool.select) return;
    final hit = _nearestPath(_toWorld(position));
    if (hit == null) return;
    setState(() {
      selectedIndex = hit;
      wholeShapeSelected = true;
      status = '已选中整个图形，拖动或捏合可整体编辑';
    });
  }
  ...
  Future<void> _showPreciseCreateDialog() async {
    ...
    _recordChange();
    setState(() {
      final createdPaths = _groupLoadedSegments([created.points]);
      paths.addAll(createdPaths);
  ...
  Future<void> _editSelected() async {
    ...
    final indices = _selectionIndices();
    final selectedPoints = indices.expand((item) => paths[item].points).toList();
    if (selectedPoints.isEmpty) return;
    final minX = selectedPoints.map((point) => point.dx).reduce(math.min);
    final maxX = selectedPoints.map((point) => point.dx).reduce(math.max);
    final minY = selectedPoints.map((point) => point.dy).reduce(math.min);
    final maxY = selectedPoints.map((point) => point.dy).reduce(math.max);
    ...
    setState(() {
      for (final item in indices) {
        paths[item].points = paths[item].points
            .map(
              (point) => Offset(
                oldWidth == 0
                    ? values[0]
      ...
      status = indices.length > 1 ? '整体图形尺寸已更新' : '对象尺寸已更新';
    });
  }
  ...
  Future<void> _scaleSelected() async {
    ...
    final indices = _selectionIndices();
    final points = indices.expand((item) => paths[item].points).toList();
    ...
    controller.dispose();
    if (!mounted || percentage == null || percentage == 100) return;
    _recordChange();
    ...
    setState(() {
      ...
      status = wholeShapeSelected
          ? '已按 ${percentage.toStringAsFixed(1)}% 缩放整个图形'
          : '已按 ${percentage.toStringAsFixed(1)}% 缩放选中线段';
    });
  }
  ...
  Future<Map<String, dynamic>> _get(String endpoint) async {
    if (widget.previewMode) {
      throw StateError('布局预览模式不会读取小车图纸');
    }
    final client = HttpClient()..connectionTimeout = const Duration(seconds: 5);
    try {
  ...
  Offset? _pointFromRaw(Object? raw) {
    if (raw is List && raw.length >= 2 && raw[0] is num && raw[1] is num) {
      return Offset((raw[0] as num).toDouble(), (raw[1] as num).toDouble());
    }
    if (raw is Map) {
      final value = Map<String, dynamic>.from(raw);
      final x = value['x'] ?? value['position_x'];
      final y = value['y'] ?? value['position_y'];
      if (x is num && y is num) return Offset(x.toDouble(), y.toDouble());
    }
    return null;
  }

  List<_SketchPath> _pathsFromDrawing(Map<String, dynamic> payload) {
    final pointPaths = <List<Offset>>[];
    final rawSegments = payload['segments'];
    if (rawSegments is List) {
      for (final raw in rawSegments) {
        if (raw is! Map) continue;
        final segment = Map<String, dynamic>.from(raw);
        final startX = segment['start_x'];
        final startY = segment['start_y'];
        final endX = segment['end_x'];
        final endY = segment['end_y'];
        if (startX is num && startY is num && endX is num && endY is num) {
          pointPaths.add([
      ...
      if (pointPaths.isNotEmpty) {
        final metadata = payload['path_metadata'];
        if (metadata is List) {
          final restored = _pathsFromSegmentMetadata(pointPaths, metadata);
          if (restored.isNotEmpty) return restored;
        }
        return _groupLoadedSegments(pointPaths);
      }
    }

    final rawPaths = payload['planned_paths'] ?? payload['paths'];
    if (rawPaths is! List) return const [];
    for (final raw in rawPaths) {
      if (raw is! Map) continue;
      final path = Map<String, dynamic>.from(raw);
      if (path['route_type'] == 'transition') continue;
      final points = (path['points'] as List? ?? const [])
          .map(_pointFromRaw)
          .whereType<Offset>()
          .toList();
      if (points.length > 1) pointPaths.add(points);
    }
    return _groupLoadedSegments(pointPaths);
  }

  List<_SketchPath> _pathsFromSegmentMetadata(
    List<List<Offset>> segments,
    List metadata,
  ) {
    final restored = <_SketchPath>[];
    for (final raw in metadata) {
      if (raw is! Map) continue;
      final item = Map<String, dynamic>.from(raw);
      final start = (item['segment_start'] as num?)?.toInt();
      final count = (item['segment_count'] as num?)?.toInt();
      if (start == null || count == null || start < 0 || count <= 0 ||
          start + count > segments.length) {
        continue;
      }
      final groupId = _nextShapeGroupId++;
      for (var index = start; index < start + count; index++) {
        restored.add(_SketchPath(
          List<Offset>.from(segments[index]),
          groupId: groupId,
        ));
      }
    }
    return restored.length == segments.length ? restored : const [];
  }

  List<_SketchPath> _groupLoadedSegments(
    List<List<Offset>> pointPaths, {
    bool keepTogether = false,
  }) {
    final segments = <List<Offset>>[];
    for (var group = 0; group < pointPaths.length; group++) {
      final points = pointPaths[group];
      for (var index = 1; index < points.length; index++) {
        if ((points[index] - points[index - 1]).distance > 0.000001) {
    ...
    if (keepTogether) {
      final groupId = _nextShapeGroupId++;
      return [
        for (final segment in segments) _SketchPath(segment, groupId: groupId),
      ];
    }
    final parent = List<int>.generate(segments.length, (index) => index);
    int root(int value) {
    ...
    for (var left = 0; left < segments.length; left++) {
      for (var right = left + 1; right < segments.length; right++) {
        final touches = segments[left].any(
          (a) => segments[right].any((b) => (a - b).distance <= 0.000001),
        );
        if (touches) join(left, right);
      }
    }

    // Old saved drawings do not carry template group ids.  Treat components
    // enclosed by an outline as part of that outline, such as court markings.
    Map<int, List<Offset>> componentPoints() {
      final result = <int, List<Offset>>{};
      for (var index = 0; index < segments.length; index++) {
        result.putIfAbsent(root(index), () => []).addAll(segments[index]);
      }
      return result;
    }

    bool encloses(List<Offset> outer, List<Offset> inner) {
      const tolerance = 0.0001;
      final outerMinX = outer.map((point) => point.dx).reduce(math.min);
      final outerMaxX = outer.map((point) => point.dx).reduce(math.max);
      final outerMinY = outer.map((point) => point.dy).reduce(math.min);
      final outerMaxY = outer.map((point) => point.dy).reduce(math.max);
      final innerMinX = inner.map((point) => point.dx).reduce(math.min);
      final innerMaxX = inner.map((point) => point.dx).reduce(math.max);
      final innerMinY = inner.map((point) => point.dy).reduce(math.min);
      final innerMaxY = inner.map((point) => point.dy).reduce(math.max);
    ...
    final components = componentPoints();
    final componentRoots = components.keys.toList();
    for (var left = 0; left < componentRoots.length; left++) {
      for (var right = left + 1; right < componentRoots.length; right++) {
        final leftRoot = componentRoots[left];
        final rightRoot = componentRoots[right];
        if (encloses(components[leftRoot]!, components[rightRoot]!) ||
            encloses(components[rightRoot]!, components[leftRoot]!)) {
          join(leftRoot, rightRoot);
        }
      }
    }
    ...
    return [
      for (var index = 0; index < segments.length; index++)
        _SketchPath(
          segments[index],
          groupId: groupIds.putIfAbsent(root(index), () => _nextShapeGroupId++),
        ),
    ];
  }
  ...
  Future<void> _loadExistingDrawing() async {
    final fileName = editingFileName!;
    if (widget.previewMode) {
      setState(() => status = '预览模式不能读取小车上的已有图纸');
      return;
    }
    setState(() {
      busy = true;
      status = '正在读取 $fileName...';
    });
    try {
      final result = await _get(
        '/api/drawings/${Uri.encodeComponent(fileName)}',
      );
      final loaded = result['ok'] == true
          ? _pathsFromDrawing(result)
          : const <_SketchPath>[];
      if (!mounted) return;
      if (loaded.isEmpty) {
        setState(() {
          status =
              ...
              '图纸不包含可编辑图元；后端需返回 segments 或 planned_paths';
        });
        return;
      }
      setState(() {
        ...
        undoStack.clear();
        redoStack.clear();
        status = '已载入 ${loaded.length} 个可编辑路径，修改后可覆盖保存';
      });
    } catch (_) {
      if (mounted) {
        setState(() => status = '读取图纸失败，请检查小车连接与图纸读取接口');
      }
    } finally {
  ...
  Future<void> _chooseExistingDrawing(String? fileName) async {
    if (fileName == null || fileName == editingFileName) return;
    if (busy) return;
    setState(() {
  ...
  Future<void> _generate({bool replaceSelected = false}) async {
    ...
    try {
      ...
      setState(() {
        ...
        final generatedSegments = _groupLoadedSegments(
          generated.map((path) => path.points).toList(),
        );
        if (replaceSelected && selectedIndex != null && generated.isNotEmpty) {
          final index = selectedIndex!;
          paths
            ..removeAt(index)
            ..insertAll(index, generatedSegments);
          selectedIndex = index;
          status = 'AI 已局部重绘选中对象';
        } else {
          paths.addAll(generatedSegments);
          selectedIndex = null;
  ...
  List<Map<String, Object?>> _pathMetadata() {
    var segmentStart = 0;
    final result = <Map<String, Object?>>[];
    for (var index = 0; index < paths.length; index++) {
      final path = paths[index];
      final count = math.max(0, path.points.length - 1);
      result.add({
        'path_index': index,
        'segment_start': segmentStart,
        'segment_count': count,
        'closed': path.points.length > 2 &&
            (path.points.first - path.points.last).distance < 0.001,
        if (path.groupId != null) 'group_id': path.groupId,
      });
      segmentStart += count;
    }
    return result;
  ...
  Future<void> _save() async {
    ...
    if (_editingExistingDrawing) {
      final overwrite = await showDialog<bool>(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('保存图纸新版本'),
          content: Text('将使用修改后的图元生成 $editingFileName 的新版本，原版本保留。'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context, false),
              child: const Text('取消'),
      ...
      if (overwrite == true) name = _editableBaseName;
    } else {
    ...
    try {
      ...
      if (result['ok'] == true) {
        ...
        if (!mounted) return;
        final savedFile = result['file_name']?.toString();
        _hasUnsavedChanges = false;
        Navigator.pop(
          context,
          savedFile?.isNotEmpty == true ? savedFile : editingFileName ?? name,
        );
      } else {
  ...
  void _applyTemplate(String template, String label) {
    ...
    final generated = _templatePaths(template)
        .map(
          (path) =>
              _SketchPath(path.points.map((point) => point + offset).toList()),
        )
        .toList();
  ...
  Widget build(BuildContext context) {
    ...
    final appBar = AppBar(
      title: Text(_editingExistingDrawing ? '编辑 $editingFileName' : '图纸编辑器'),
      actions: [
        IconButton(
          tooltip: '撤销',
        ...
        IconButton(
          tooltip: '缩放选中图形',
          onPressed: selectedIndex == null || busy ? null : _scaleSelected,
        ...
        if (widget.existingFileNames.isNotEmpty)
          Padding(
            ...
            child: DropdownButtonFormField<String>(
              initialValue: widget.existingFileNames.contains(editingFileName)
                  ? editingFileName
                  : null,
              isExpanded: true,
              ...
              hint: const Text('新建图纸 · 选择已有图纸可直接编辑'),
              items: widget.existingFileNames
                  .map(
                  ...
                  .toList(),
              onChanged: busy ? null : _chooseExistingDrawing,
            ),
          ),
        Container(
                ...
                children: [
                  CustomPaint(
                    painter: _DrawingCanvasPainter(
                      ...
                      draft: draft,
                      selectedIndices: _selectionIndices().toSet(),
    ...
    final content = LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Row(
          ...
          children: [
            SizedBox(
              width: templateLibraryExpanded ? 310 : 52,
              child: _DrawingTemplateLibrary(
                expanded: templateLibraryExpanded,
                onToggle: () => setState(
                  () => templateLibraryExpanded = !templateLibraryExpanded,
                ),
    ...
    return PopScope<bool>(
      canPop: !_hasUnsavedChanges && !busy,
      onPopInvokedWithResult: (didPop, result) async {
        if (didPop || busy || !_hasUnsavedChanges || !mounted) return;
        final discard = await showDialog<bool>(
          context: context,
          builder: (context) => AlertDialog(
            title: const Text('放弃未保存修改？'),
            content: const Text('当前图纸有尚未保存的编辑内容。确定要退出吗？'),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(context, false),
...
class _DrawingTemplateLibraryState extends State<_DrawingTemplateLibrary> {
  ...
  Widget _buildExpanded(BuildContext context) => Padding(
    padding: EdgeInsets.zero,
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
  ...
  void _drawContent(Canvas canvas, Size size) {
    ...
    for (var index = 0; index < paths.length; index++) {
      drawPath(
        ...
        selectedIndices.contains(index) ? 4 : 3,
      );
    }
    for (var pathIndex = 0; pathIndex < paths.length; pathIndex++) {
      final path = paths[pathIndex];
