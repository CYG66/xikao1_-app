...
flowchart LR
    ...
    B -->|rclpy / ROS2 服务与话题| R[xline_cyg ROS2]
    R --> C[CAN 底盘]
    R --> P[喷码机]
    R --> L[IMU / 里程计 / 全站仪]
    B --> D[(SQLite 项目与审计数据库)]
...
- `xline_cyg` 负责底盘、定位、规划、跟踪、速度仲裁和喷码机驱动。
...
| `D:\xikao\xline_cyg` | 本地 ROS2 源码参考 | 默认只读，不得随 App 后端修改 |
| `/home/qingz/xline_app_backend` | 小车运行后端 | 只按最终同步清单替换 |
| `/home/qingz/xline_cyg` | 小车 ROS2 运行代码 | 未经明确授权禁止修改 |
...
ROS2: Humble
Backend service: xline-app-backend.service
ROS2 runtime service: xline-cyg-runtime.service
```

不要把密码、AI API Key 或其他密钥写进 Git、APK 或本文档。AI Key 应只放在小车的 `/home/qingz/.config/xline-agent.env`。
...
确认测试通过后，再按同步清单将最终文件传到 `/home/qingz/xline_app_backend`。不要同步 `tests`、缓存、临时文件或本地数据库。
...
sleep 3
sudo systemctl status xline-app-backend --no-pager -l
ss -lntp | grep ':8000'
...
2. 确认 Git 分支与工作区状态，不要使用 `git reset --hard` 覆盖未确认改动。
3. 在本地运行后端编译和 90 项测试。
4. 使用真车构建参数生成 APK，并确认首页不显示“布局预览”。
5. SSH 登录小车，检查两个 systemd 服务、8000 端口和 `/api/status`。
6. 比较本地/远端后端文件哈希，再决定是否同步。
7. 先验证急停与底盘低速，再验证喷码机，最后运行完整 Agent 项目闭环。
